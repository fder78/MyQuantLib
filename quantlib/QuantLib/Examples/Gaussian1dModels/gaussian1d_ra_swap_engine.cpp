...
namespace QuantLib {
    ...
    const Real
    G1d_RA_Swap_Engine::underlyingNpv(const Date &expiry,
                                                       const Real y) const {
    ...
    G1d_RA_Swap_Engine::underlyingType() const {
        return arguments_.swap->type();
    }

    const Date G1d_RA_Swap_Engine::underlyingLastDate() const {
        return arguments_.fixedPayDates.back();
    }

    const Disposable<Array> G1d_RA_Swap_Engine::initialGuess(
    ...
    void G1d_RA_Swap_Engine::calculate() const {
        ...
        boost::shared_ptr<RebatedExercise> rebatedExercise = boost::dynamic_pointer_cast<RebatedExercise>(arguments_.exercise);

        int idx = arguments_.exercise->dates().size() - 1;
        int minIdxAlive = static_cast<int>(std::upper_bound(arguments_.exercise->dates().begin(), arguments_.exercise->dates().end(), settlement) -
        ...
        do {
            ...
            for (Size k = 0; k < (expiry0 > settlement ? npv0.size() : 1); k++) {
                ...
                if (expiry1Time != Null<Real>()) {
                    ...
                    Array yg = model_->yGrid(stddevs_, integrationPoints_, expiry1Time, expiry0Time, expiry0 > settlement ? z[k] : 0.0);
                ...
                if (expiry0 > settlement) {
                    ...
                    for (Size l = k1; l < arguments_.floatingCoupons.size(); l++) {
                        Real zSpreadDf = oas_.empty() ? 1.0 : 
							std::exp(-oas_->value() * (model_->termStructure()->dayCounter().yearFraction(expiry0, arguments_.floatingPayDates[l])));
                        Real amount;
                        if (arguments_.floatingIsRedemptionFlow[l])
                            ...
                            amount = arguments_.floatingNominal[l] *
                                     ...
                                     (arguments_.floatingGearings[l] *
                                          model_->forwardRate(arguments_.floatingFixingDates[l], expiry0, z[k], arguments_.swap->iborIndex()) + arguments_.floatingSpreads[l]);
                        floatingLegNpv += amount * model_->zerobond(arguments_.floatingPayDates[l], expiry0, z[k], discountCurve_) * zSpreadDf;
                    }
					////fixed leg amount
                    Real fixedLegNpv = 0.0;
                    for (Size l = j1; l < arguments_.fixedCoupons.size(); l++) {
                        Real zSpreadDf = oas_.empty() ? 1.0 : 
							std::exp(-oas_->value() * (model_->termStructure()->dayCounter().yearFraction(expiry0, arguments_.fixedPayDates[l])));
                        fixedLegNpv += arguments_.fixedCoupons[l] * model_->zerobond(arguments_.fixedPayDates[l], expiry0, z[k], discountCurve_) * zSpreadDf;
                    }
                    Real rebate = 0.0;
                    Real zSpreadDf = 1.0;
                    ...
                    if (rebatedExercise != NULL) {
                        ...
                        zSpreadDf = oas_.empty() ? 1.0 :
							std::exp(-oas_->value() * (model_->termStructure()->dayCounter().yearFraction(expiry0, rebateDate)));
                    }
                    Real exerciseValue =
                        ((type == Option::Call ? 1.0 : -1.0) * (floatingLegNpv - fixedLegNpv) 
							+ rebate * model_->zerobond(rebateDate, expiry0, z[k], discountCurve_) * zSpreadDf) / model_->numeraire(expiry0Time, z[k], discountCurve_);
