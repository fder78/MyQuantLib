...
namespace QuantLib {
    ...
    class RAFloatSwap : public FloatFloatSwap {
      public:
        ...
        RAFloatSwap(
			...
			const Real cappedRate1 = Null<Real>(),
            const Real flooredRate1 = Null<Real>(), 
			const Real gearing2 = 1.0,
            const Real spread2 = 0.0, 
			const Real cappedRate2 = Null<Real>(),
		...
		std::vector<std::pair<Real, Real> > getRange() const { return indexRange_; }
		std::vector<Rate> getFixedRate() const { return fixedRate_; }

      private:
        void setupExpired() const;
		std::vector<std::pair<Real,Real> > indexRange_;
		std::vector<Rate> fixedRate_;
    ...
    class RAFloatSwap::arguments : public FloatFloatSwap::arguments {
      public:
		  void validate() const;
		  std::vector<std::pair<Real, Real> > indexRange;
