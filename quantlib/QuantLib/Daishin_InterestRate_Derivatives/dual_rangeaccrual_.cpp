...
namespace QuantLib {
	...
	std::vector<Real> dual_rangeaccrual_fdm(Date evaluationDate,
		...
		Date firstCallDate,
		Real pastAccrual,
		boost::shared_ptr<Generalized_HullWhite> hw1,
		boost::shared_ptr<Generalized_HullWhite> hw2,
		...
		Real rho, Size tGrid, Size rGrid) {
			...
			boost::shared_ptr<IborIndex> index(new Euribor3M(Handle<YieldTermStructure>( discTS.currentLink() )));

			std::vector<Real> gearing(1, 0.0);
			...
			boost::shared_ptr<Generalized_HullWhite> obs1Model(new 
				Generalized_HullWhite(hw1->termStructure(), obs1GenHWParams.tenor, obs1GenHWParams.sigma, obs1GenHWParams.a, obs1FXVol, obs1FXCorr ) );

			boost::shared_ptr<Generalized_HullWhite> obs2Model(new 
				Generalized_HullWhite(hw2->termStructure(), obs2GenHWParams.tenor, obs2GenHWParams.sigma, obs2GenHWParams.a, obs2FXVol, obs2FXCorr ) );

			/*****Pricing Engine*****/

			boost::shared_ptr<PricingEngine> engine(new Fdm_R2_Dual_RA_Engine(hw1, hw2, discTS, rho, pastAccrual, tGrid, rGrid, rGrid));
