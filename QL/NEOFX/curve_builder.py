...
# global data
def buildCurve(todaysDate, settlementDate, deposits, futures, swaps, calendar, dc):
    ...
    lastDate = todaysDate + deposits["quote"][-1][0]
    months = futures["maturity"]
    fdc = deposits["dc"]
    fbdc = deposits["bdc"]
    futuresHelpers = []
    for n, unit in futures["quote"]:
        if (n+Period(months,Months)>lastDate):
    ...
    if len(futures["quote"])>0:
        lastDate = futures["quote"][-1][0] + Period(months,Months)
    ...
    depoFuturesSwapCurve = PiecewiseFlatForward(settlementDate, helpers, dc)
